...
async function attach(todoID, projID) {
  const res = await chai
    ...
    .send({ id: todoID });
  expect([200, 201, 409]).to.include(res.status); // 409 already linked
  return res;
}
...
async function listTodoIdsForProject(projID) {
  ...
  const ids = todos.map((t) => t.id ?? t.todo?.id).filter(Boolean);
  return ids;
}
